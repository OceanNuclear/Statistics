...
class distribution:	#note that I initiate it with σ square, i.e. vari
	def __init__(self, μ, σ_sq, σ_sq_on_mean=0):
		self.mean = μ
		self.vari = σ_sq
		self.standard_dev = sqrt(σ_sq)
		self.uncertainy_on_mean=σ_sq_on_mean
	...
	def __truediv__(self,Y):
		...
		return newDist
class combinedDist:
	def __init__(self, func, dist1, dist2):
		assert type(dist1)==distribution
		assert type(dist2)==distribution
		self.func=func
		self.dist1=dist1
		self.dist2=dist2
		self.mean = func(dist1.mean,dist2.mean)
...
def sampleToDist(dataList):
	...
	newDist = distribution(new_mean, new_vari, σ_sq_on_mean = uncertainy_on_mean)
	return newDist
